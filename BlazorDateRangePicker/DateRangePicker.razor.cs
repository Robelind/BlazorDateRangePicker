...
public partial class DateRangePicker : ComponentBase, IConfigurableOptions
{
    ...
    public virtual async Task ClickDate(DateTimeOffset date)
    {
        ...
        {
            DateTimeOffset dateInSpan = TStartDate!.Value;
            bool setEnd = true;

            while (dateInSpan <= date)
            {
                if (DaysEnabledFunction != null && !DaysEnabledFunction.Invoke(dateInSpan) ||
                    (DaysEnabledFunctionAsync != null && !await DaysEnabledFunctionAsync.Invoke(dateInSpan)))
                {
                    setEnd = false;
                    break;
                }
                dateInSpan += TimeSpan.FromDays(1);
            }
